...
## Tool: MAX Messenger (max-mcp)

This project has the MCP server **max-mcp** — direct access to a MAX (max.ru) account via a user
session.

> Full tool names in Claude Code are prefixed `mcp__max-mcp__` — e.g. `mcp__max-mcp__list_chats`.
> The prefix is omitted in the table below for brevity.

### Available tools

| Tool | What it does |
|---|---|
| `list_chats(limit, marker)` | List all chats and channels. Pagination via `next_marker`. |
| `get_chat(chat_id)` | Full chat/channel info: name, type, members, description. |
| `read_messages(chat_id, limit, before_time)` | Message history. Backward pagination via `next_before_time`. |
| `search_messages(query, chat_id, limit, scan_limit)` | Text search (client-side, no server support). |
| `list_channel_posts(channel_id, limit, before_time)` | Channel posts with attachments, reactions, stats. |
| `dump_channel(channel_id, since_time, max_posts)` | Bulk post export (max 1000 per call). |
| `send_message(chat_id, text, reply_to_id)` | Send a text message. |
| `send_file(chat_id, file_path, caption)` | Send a file/photo/video. |

### API specifics (must account for)

- **IDs** — numeric (`int`). Channels have large negative IDs.
- **`time`** — unix timestamp in **milliseconds** (not seconds).
- **`sender`** — numeric user ID, no name returned.
- **`chat_id` in messages** — may be `null` in DMs (pymax 2.1.2 quirk).
- **Pagination** — always backward by time. Pass `next_before_time` from the previous response as
  `before_time` of the next call.
- **`list_chats`** — field `last_event_time` (not `last_message_at`), `participants_count` (not
  `members_count`).
- **`search_messages`** — no native search in MAX, scans history client-side. For deep search raise
  `scan_limit` (default 500).
- **`dump_channel`** — for a full archive, call in a loop passing the previous batch's `min(time)` as
  `before_time`.
- **`send_file`** — takes an absolute path. Only files under `~/Downloads`, `~/Documents`, `/tmp` are
  allowed. Symlinks and paths to `~/.ssh`, `~/.aws`, etc. are blocked.
- **`stopped_reason`** in `dump_channel`: `max_posts` | `since_time` | `exhausted`.

### Typical scenarios

**Find a channel and export latest posts:**
```
1. list_chats() → find the channel_id
2. dump_channel(channel_id=X, max_posts=100)
```

**Monitor mentions in a chat:**
```
search_messages(query="ключевое слово", chat_id=X, scan_limit=2000)
```

**Publish a post:**
```
send_message(chat_id=X, text="текст поста")
```

**Send an image:**
```
send_file(chat_id=X, file_path="~/Downloads/post.jpg", caption="подпись")
```

### Technical
- Code: `/path/to/max-mcp/` (a separate, self-hosted MCP server for the MAX messenger; not included in this repo)
- Session: `~/.max-mcp/session.db` (login via `uv run python -m max_mcp.auth login-qr`)
